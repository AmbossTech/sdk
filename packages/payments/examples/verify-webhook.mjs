/**
 * Real ESM example for @ambosstech/payments' webhook verification — and the
 * CI check for the ESM half of the dual build (see verify-webhook.cjs for
 * the CJS half). Unlike send.ts/receive.ts this needs no live API key:
 * HMAC verification is fully offline, so it can run in CI against nothing
 * but the built `dist/` output.
 *
...
console.log('✅ ESM: @ambosstech/payments resolved from dist/ and verified a webhook');
